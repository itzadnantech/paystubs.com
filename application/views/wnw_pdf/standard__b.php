...
<table style="width: 100%;border-spacing:0;background-color:#deefdd;padding: 0;">
	<tr>
		<td style="padding:10px 10px;font-family: arial;text-align:left;"><span style="color:#7dac42;"> Company Name : </span><?= $postedData['uk__emloyee__officeaddress'] ?> </td>
...
<?php
                        ...
                        if($period_deductions_count >$pay_method_deductions_count){
                         ...
                         // echo $extra_row; exit;
                            if($pay_method_deductions_count < 4 && $period_deductions_count < 4 ){
                            	...
                            	if($period_deductions_count < 7){
                            		$extra_row_period=7-$period_deductions_count;  
                            	}
                            	if($extra_row_pay_method < 7){
                           	...
                           	if($total_row < 7) {
                           		if($pay_method_deductions_count > $period_deductions_count){
                         		 $ni_number_extra=7-4;
									}else{
                         			$ni_number_extra=7-4;
                         		}
	...
	/*						if($pay_method_deductions_count > $period_deductions_count){
									...
									$extra_row_period =  $pay_method_deductions_count-$period_deductions_count;
							} 
							if($pay_method_deductions_count < $period_deductions_count){
								
									$extra_row_pay_method =  $period_deductions_count-$pay_method_deductions_count;
									$extra_row_period = $period_deductions_count-$pay_method_deductions_count;
							} 
*/
		...
		<tr>
			<td style="border-spacing:0px;overflow: hidden;perspective: 1px;border: 1px solid #7ebc59;width: 50%;float: left;display: inline-block;vertical-align:top;padding: 0;    background-image: linear-gradient(to right, #e4efed, #d6e8d0);">
				...
				<table  style="width: 100%;border-spacing:0;border-radius: 15px;overflow: hidden;perspective: 1px;">
					<tr>
						<th colspan="4" style="text-align:center;background-color:#deefdd;font-weight:400;font-size:17px;">NI NUMBER - <?= $postedData['uk__emloyee__nicno'] ?></th>
					</tr>
			...
			<table  style="width: 100%;border-spacing:0;border-radius: 15px;overflow: hidden;perspective: 1px;">
				<tr>
					<th colspan="2" style="text-align:center;background-color:#deefdd;font-weight:400;font-size:17px;"><?= (isset($postedData['uk__emloyee__pay_period_month']) && !empty($postedData['uk__emloyee__pay_period_month'])  ) ? $postedData['uk__emloyee__pay_period_month'] : 'Pay Period - Month'  ?>
					   </th>
				</tr>
			...
			<table  style="width: 100%;border-spacing:0;border-radius: 15px;overflow: hidden;">
				<tr>
					<th colspan="2" style="font-size:17px;text-align:center;background-color:#deefdd;font-weight:400;"><?= (isset($postedData['uk__emloyee__pay_period_bank']) && !empty($postedData['uk__emloyee__pay_period_bank'])) ? $postedData['uk__emloyee__pay_period_bank'] : 'Pay Method - Bank'  ?>
                       </th>
				</tr>
				<tr style="background-color:#7ebc59;width:100%;">
			...
			<td style="overflow: hidden;perspective: 1px;width: 100%;display: inline-block;    padding: 0;">
				<table  style="width: 100%;border-spacing:0;border-radius: 15px;overflow: hidden;perspective: 1px;    border-radius: 13px;">
					...
					<tr  style="background-color:#dbeedb;">
						...
						<td  style="padding: 5px 10px;font-family: arial;text-align:center;font-weight:bold;color:#474d49;">
