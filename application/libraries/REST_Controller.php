...
abstract class REST_Controller extends CI_Controller {
    ...
    public function validate_basic_auth($validate = true){
       
        if($validate){
            
           $authorization_key =  $this->config->item('authorization_key');
           $auth_password = $this->input->server('PHP_AUTH_PW');
           $auth_username =  $this->input->server('PHP_AUTH_USER');
           
           $encode_username_password =  base64_encode($auth_username.":".$auth_password);
           if($encode_username_password == $authorization_key){
               return true;
           }else{
               return false;
           }
