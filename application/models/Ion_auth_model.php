...
class Ion_auth_model extends CI_Model
{
	...
	public function custom_Text($username)
	{
	...
	public function login($identity, $password, $remember = FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', email, id, password, active, last_login, first_name, last_name,activation_code')
			->where($this->identity_column, $identity)
			->limit(1)
			->order_by('id', 'desc')
			->get($this->tables['users']);
	 ...
	 * Email Users 
	 */
	public function emailusers($groups = NULL)
	{
		$SQL = 'SELECT u.id,u.first_name,u.last_name,u.company,u.email,u.phone,u.active,us.paystub_country_id,pc.name as country_name FROM `users` as u,users_subscription us,paystub_countries pc where u.id=us.user_id and  us.paystub_country_id <> 0 and us.paystub_country_id=pc.id';
		$query = $this->db->query($SQL);
		$result = $query->result_array();
		return $result;
	}

	public function getUserByCountry($id)
	{
		$SQL = 'SELECT u.email FROM `users` as u,users_subscription us,paystub_countries pc where u.id=us.user_id and  us.paystub_country_id <> 0 and us.paystub_country_id=pc.id and pc.id = ' . $id;
		if ($id == 'all') {
			$SQL = 'SELECT u.email FROM `users` as u,users_subscription us,paystub_countries pc where u.id=us.user_id and  us.paystub_country_id <> 0 and us.paystub_country_id=pc.id';
		}
	...
	{
		$SQL = 'SELECT * FROM paystub_countries';
		$query = $this->db->query($SQL);
		$result = $query->result_array();
		return $result;
	}
	...
	public function update_user_login($id = NULL, $data = array())
	{
		$this->db->where('id', $id, 1);
		$uQuery = $this->db->update('users', $data);
		if ($uQuery) {
			return true;
		} else {
			return false;
